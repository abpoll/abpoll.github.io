...
def repo_badge(repo_url):
    ...
    return badge_link(
        repo_url,
        "https://img.shields.io/static/v1?style=flat&label=&message=Open%20Source",
        "Open Source",
    )
...
def format_authors(author_field: str, max_names: int = 4) -> str:
    ...
    If there are > max_names authors:
      ...
      - else: show first (max_names - 2), then […], then me, then et al.
    ...
    # show first (max_names - 2), then […], then me, then et al.
    # e.g. max_names=4 => first 2, […], me, et al.
    head_count = max(1, max_names - 2)
    head = [fmt_name(a) for a in authors[:head_count]]
    me = fmt_name(authors[me_idx])

    shown = head + ["<span class='pub-ellipsis'>[…]</span>", me, "et al."]
    return ", ".join(shown)
